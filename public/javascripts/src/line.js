...
// 								// if((event.pageX - objContOffset.left) < (reAnchParRect.left - objContOffset.left) && (event.pageY - objContOffset.top) < (reAnchParRect.top - objContOffset.top) || (event.pageY - objContOffset.top) > (reAnchParRect.bottom - objContOffset.top)){
// 								// 	nl1x2 = nl1x1 + 20;//May need to make the value here dependent on overall scaling
// 								// 	nl1x3 =	nl1x2;
...
function tempLineCreation(){//TO DO: when the x delta is negative there is a time where it intersects with the element
	var objContOffset = $("#object_container").offset();
	var firstAnchorRect = interact.getElementRect(event.target);
			    ...
			    function handleMouseMove(event) {
					...
					// console.log('nlXDelta = ' + nlXDelta);
						...
						if(startAnchorID == 'rightAnchor'){
							...
							if(nlXDelta >= 0){//Right anchor when x delta is positive

								if(nlXDelta < Math.abs(nlYDelta)){//Right anchor when x delta is smaller than y delta
							...
							} else if(nlXDelta < 0){//Right anchor when x delta is negative

								if(Math.abs(nlXDelta) > Math.abs(nlYDelta)){//X length is longer than Y length
								...
								if(Math.abs(nlXDelta) < Math.abs(nlYDelta)){//Y length is longer than X length

									if((event.pageY - objContOffset.top) < (reAnchParRect.top - objContOffset.top) || (event.pageY - objContOffset.top) > (reAnchParRect.bottom - objContOffset.top)){//Doesn't intersect with

										if((event.pageX - objContOffset.left) < (reAnchParRect.left - objContOffset.left)){
								...
								if((event.pageY - objContOffset.top) < (reAnchParRect.bottom - objContOffset.top) && (event.pageY - objContOffset.top) > (reAnchParRect.top - objContOffset.top) && (event.pageX - objContOffset.left) < (reAnchParRect.left - objContOffset.left)){
